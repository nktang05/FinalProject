...
# method to get population data as data tablepopulation
def get_pop_data():
    # get api data using key
    ...
    # adjust data table format and combine all individual tables together
    ...
    # method to put data into sql database
...
# method to transpose database table
def transpose():
    conn = sqlite3.connect('tang.db', detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
    cur = conn.cursor()

   # method to make new data table
    cur.execute('''
    CREATE TABLE IF NOT EXISTS popData (
     ...
     # join tables and add with specific headers
    cur.execute('''
